...
def XyzToInp(file, loc='./', saveloc='./', savename='', method='B3LYP',\
             basis_set='def2-SVP', opt=False, freq=False,\
             numfreq=False, newgto={}, scan=False,\
    ...
    XyzToInp(file, loc='./', saveloc='./', savename='', method='B3LYP', basis_set='def2-SVP', opt=False, freq=False, scan=False, external_force=False, ts=False, aim=[0, 0], stretch=1, scanstep=10, strain=-1, maxiter=-1, maxcore=-1, corenum=1, electron=0, state=1)
    ...
    savename: The name of your saved .inp file.
    ...
    numfreq: To show if you want to calculate the polarisation of intermolecule system and thermodynamic data. The default is False. This statement is superior to 'freq'.
    newgto: To use different basis sets for some special elements. e.g: newgto = {'Br': 'def2-TZVP', 'Fe': 'LANL2DZ'}
    ...
    Example 2:
        Input:
            from MCPoly.orcaset import XYZtoINP
            XyzToInp('Atoms1', method='B3LYP D3BJ', basis_set='def2-TZVP', opt=True,\
                     numfreq=True,maxcore=4096, corenum=8)
        Output in Atom1.inp:
            #Powered by MCPoly
           
            ! B3LYP D3BJ def2-TZVP OPT NUMFREQ
            %ELPROP
                POLAR 1
            END
            %maxcore 4096
    ...
    Example 4:
        Input:
            from MCPoly.orcaset import XYZtoINP
            XyzToInp('Atom1', method='B3LYP D3BJ', basis_set='def2-TZVP', opt=True, scan=True, \
    ...
    if savename == '':
        ...
        try:
            w = open(saveloc+savename+'.inp', 'x')
        except:
            w = open(saveloc+savename+'.inp', 'w')
    ...
    if numfreq == True:
        if opt == False:
            warnings.warn('You set the polarisation calculation'
                          + ' without geometry optimisation.')
        w.write('NUMFREQ')
    elif freq == True:
        if opt == False:
            warnings.warn('You set the frequency calculation'
                          + ' without geometry optimisation.')
        w.write('FREQ')
    w.write('\n')

    if newgto != {}:
        xele = list(newgto.keys())
        w.write('%basis\n')
        for ele in xele:
            w.write('newgto {0} "{1}" end\n'.format(ele, newgto[ele]))
        w.write('end\n')
